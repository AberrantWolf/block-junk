...
//  2. Pick the face texture (top/side/bottom by normal) from `blocks`.
//  3. For each baked layer: wrap world-space face UV by the layer's
//     period, textureLoad the tile (nearest, no sampler — tiles of
//     different periods have different pixel extents inside a fixed
//     array slot), and blend over the running color. Layer alpha is the
//     bake's paint coverage.
//  4. Apply the "finish": a cheap world-space value-noise brightness
//     jitter that never repeats, killing any residual tiling.
//
// Compositing runs in DISPLAY space (tiles are Rgba8Unorm, not Srgb) so
// it matches the CPU `bake::composite_at` byte-for-byte; the final color
// converts to linear once before PBR. The blend_rgb and finish_fbm
// functions mirror `eval::blend_rgb` / `noise::value_fbm_world` — keep
// all three in sync.
